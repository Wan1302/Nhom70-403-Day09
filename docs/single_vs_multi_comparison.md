...
| Multi-hop accuracy | 0.0% (0/3) | Chờ grading chấm | TBD | Day 08 = 0%, Day 09 có routing + MCP hỗ trợ cross-doc |
...
**Kết luận:** Day 08 thất bại hoàn toàn trên multi-hop (0%). Day 09 có cơ chế routing theo domain và MCP `search_kb` để lấy thêm chunks — kỳ vọng cải thiện, nhưng kiến trúc single-route vẫn là bottleneck khi task span 2 domain.
...
**Câu cụ thể nhóm đã debug:** Câu q15 (Ticket P1 lúc 2am, contractor Level 2 + SLA notify) ban đầu cho confidence 0.69 và answer không đề cập đủ cả 2 quy trình. Đọc trace thấy `supervisor_route=policy_tool_worker` — đúng về access policy nhưng bỏ qua SLA notification path. Xác định ngay vấn đề: câu hỏi multi-intent cần 2 worker, không phải 1. Thời gian debug: ~5 phút nhờ có trace.
...
1. **Latency & cost**: Thêm ~645ms trung bình và ~2× LLM calls. Với câu hỏi đơn giản, overhead này không mang lại lợi ích accuracy.
...
1. Cho phép supervisor gọi nhiều worker tuần tự cho multi-intent tasks (sequential chaining thay vì single-route).
